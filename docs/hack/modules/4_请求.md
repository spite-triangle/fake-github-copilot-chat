...
> **说明**：`thinking_budget` 字段已存在于原项目中，无需重复添加。仅新增 `frequency_penalty` 和 `presence_penalty`。

---

## networkRequest 函数重写 + getModelConfig 辅助函数
...
> **注意**：`agentInteractionType` 保留了项目原有的 `options.interactionTypeOverride ?? intent` 逻辑。`ILogService` 已在原文件中导入，无需额外添加。

---

##  IMakeChatRequestOptions - 新增 fetchRetryCount 字段
...
## fetchAndContinueOnLengthError - 删除 markdown 标记
...
> **说明**：`IResponseDelta` 已在原文件中导入（`import { FinishedCallback, IResponseDelta } from '../../../../platform/networking/common/fetch'`），无需额外添加。
...
- **位置**：`FilteredRetry` 返回块之后，`pendingLoggedChatRequest?.resolve(result, streamRecorder.deltas)` 之前
...
> **注意**：`workspace` 的导入已从 `import type { CancellationToken } from 'vscode'` 拆分为 `import type { CancellationToken } from 'vscode';` + `import { workspace } from 'vscode';`。`toTextParts` 已在原文件中导入。
