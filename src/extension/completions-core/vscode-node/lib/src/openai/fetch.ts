...
/**
 * Adds comment markers to text based on language ID.
 * @param text - The text to be commented
 * @param languageId - Language identifier (case-insensitive)
 * @returns Text with comment markers added
 */
	...
	async fetchAndStreamCompletions(
...

		{
			...
			if (inline_mode === "api") {
				...
				logger.debug(this.logTargetService, req.prompt as string);
			...
			// Schedule the actual network call with rate limiting
			...
			let res = await new Promise<ReturnType<typeof this.fetchService.fetch>>(async (resolve, reject) => {
				...
				if (waitTime > 0) {
					logger.debug(this.logTargetService, `Delaying request ${ourRequestId} for ${waitTime}ms to respect request interval`);
				}
